...
class timthumb
{
    ...
    protected function puockIsSignedExternalRequest(): bool
    {
        if (!$this->isURL) return false;
        if (!defined('PUOCK_TIMTHUMB_REQUIRE_SIG') || !PUOCK_TIMTHUMB_REQUIRE_SIG) return false;
        if (!defined('PUOCK_TIMTHUMB_KEY') || !is_string(PUOCK_TIMTHUMB_KEY) || PUOCK_TIMTHUMB_KEY === '') return false;

        $sig = (string)$this->param('sig', '');
        if ($sig === '') return false;

        $srcParam = (string)$this->param('src', '');
        if ($srcParam === '') return false;
        if (strpos($srcParam, "http://") === false && strpos($srcParam, "https://") === false) {
            $srcParam = (string)self::safe_base64_decode($srcParam);
        }

        $w = (int)$this->param('w', 0);
        $h = (int)$this->param('h', 0);
        $zc = (int)$this->param('zc', DEFAULT_ZC);
        $q = (int)$this->param('q', DEFAULT_Q);
        $payload = $srcParam . '|' . $w . '|' . $h . '|' . $zc . '|' . $q;
        $expected = hash_hmac('sha256', $payload, PUOCK_TIMTHUMB_KEY);
